...
async def who_is(client, message):
    ...
    message_out_str += f"<b>➲Usᴇʀ Lɪɴᴋ:</b> <a href='tg://user?id={from_user.id}'><b>Click Here</b></a>\n"
...
async def imdb_search(client, message):
    if ' ' in message.text:
        k = await message.reply('Sᴇᴀʀᴄʜɪɴɢ IᴍDB')
        r, title = message.text.split(None, 1)
        movies = await get_poster(title, bulk=True)
        if not movies:
            return await message.reply("Nᴏ Rᴇsᴜʟᴛ Fᴏᴜɴᴅ")
        btn = [
            [
        ...
        await k.edit('ɪ ғᴏᴜɴᴅ ᴏɴ ɪᴍᴅʙ', reply_markup=InlineKeyboardMarkup(btn))
    else:
